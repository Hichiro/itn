...
# 1.5 Công cụ tải và giải nén
for cmd in curl wget awk unzip; do
...
download_release() {
    mkdir -p "$INSTALL_DIR"
    cd "$INSTALL_DIR"
    # Sửa đổi cơ chế tải: Lấy file source .zip chính thức từ GitHub
    DOWNLOAD_URL="https://github.com/${GH_USER_REPO}/archive/refs/heads/main.zip"
    echo "⬇️  Tải mã nguồn từ $DOWNLOAD_URL …"
    wget -qO openclaw-source.zip "$DOWNLOAD_URL"
    unzip -qo openclaw-source.zip
    # Di chuyển source code ra thư mục chính và dọn dẹp
    mv itn-main/projects/openclaw/* . 2>/dev/null || mv itn-main/* . 2>/dev/null
    rm -rf openclaw-source.zip itn-main
}
...
install_service() {
    # Tạo thư mục bin toàn cục của user nếu chưa có
    USER_BIN_DIR="$REAL_HOME/.local/bin"
    mkdir -p "$USER_BIN_DIR"

    # Giải pháp tối ưu: Tạo liên kết mềm hệ thống thay cho lệnh pnpm cài global lỗi
    ln -sf "$INSTALL_DIR/dist/index.js" "$USER_BIN_DIR/openclaw"
    chmod +x "$INSTALL_DIR/dist/index.js" 2>/dev/null || true

    # Đảm bảo PATH nhận diện được CLI mới tạo
...
# Giới hạn dung lượng RAM Node.js tối đa 300MB để không làm treo VPS
