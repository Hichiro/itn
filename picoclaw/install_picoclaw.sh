...
# Chỉ khởi tạo các thư mục thuần Go và PicoClaw
mkdir -p $HOME/go/bin
...
elif [ "$MY_REMOTE_COMMIT" = "$LOCAL_COMMIT" ] && [ -f "$HOME/go/bin/picoclaw-termux" ]; then
...
if [ "$NEED_UPDATE" = true ]; then
    ...
    pkill -f picoclaw-termux > /dev/null 2>&1
    killall picoclaw-termux > /dev/null 2>&1
    sleep 1 

    echo "Dang tai file binary picoclaw-termux tu GitHub..."
    curl -fsSL "https://raw.githubusercontent.com/Hichiro/itn/main/picoclaw/picoclaw-termux" -o $HOME/go/bin/picoclaw-termux

    if [ $? -eq 0 ] && [ -s "$HOME/go/bin/picoclaw-termux" ]; then
...
chmod +x $HOME/go/bin/picoclaw-termux
...
# Chỉ thêm script tự khởi chạy cho picoclaw-termux gateway, không dính cấu hình cũ
if ! grep -q 'picoclaw-termux gateway' ~/.bashrc; then
    cat << 'PICOCLAW_BOOT' >> ~/.bashrc

# Tự động khởi động PicoClaw gateway ngầm nếu đã có file config.json và chưa chạy
if [ -f "$HOME/.picoclaw/config.json" ] && ! pgrep -f "picoclaw-termux gateway" > /dev/null; then
    nohup picoclaw-termux gateway > /dev/null 2>&1 &
fi
PICOCLAW_BOOT
...
if [ -f "$HOME/.picoclaw/config.json" ]; then
    if ! pgrep -f "picoclaw-termux gateway" > /dev/null; then
